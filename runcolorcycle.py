...
# One Cycle with one step and a pause of theee seconds. Hence three seconds of white light
        ...
        MY_CYCLE = colorschemes.Solid(num_led=NUM_LED, pause_value=5,
                             num_steps_per_cycle=1, num_cycles=1)
        MY_CYCLE.start()
        
# Go twice around the clock
        print('Go twice around the clock')
        MY_CYCLE = colorschemes.RoundAndRound(num_led=NUM_LED, pause_value=.05,
                                     num_steps_per_cycle=NUM_LED, num_cycles=5)
        MY_CYCLE.start()

# One cycle of red, green and blue each
        print('One strandtest of red, green and blue each')
        MY_CYCLE = colorschemes.StrandTest(num_led=NUM_LED, pause_value=.05,
                                  num_steps_per_cycle=NUM_LED, num_cycles=5)
        MY_CYCLE.start()
...
# One slow trip through the rainbow
        ...
        MY_CYCLE = colorschemes.Rainbow(num_led=NUM_LED, pause_value=0,
                               num_steps_per_cycle=255, num_cycles=10)
        MY_CYCLE.start()
